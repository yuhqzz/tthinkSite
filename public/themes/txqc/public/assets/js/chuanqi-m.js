...
/*	var _mun1 = 0;
	var _mun2 = 0;
	var _mun3 = 0;
	function dealerLod(obj, _mun){
		$(obj).on('click', function(){
			var $this = $(this);
			var _length = $this.parent().find('li').length;
			var _lengthHover = $this.parent().find('li.hover').length;
			var _display = 6;
			function _iSlengthHover(){
				_lengthHover = $this.parent().find('li.hover').length;
				if(_lengthHover == _length){
					$this.html('已加载完');
					return false;
				}
			}
			...
			_mun++
			_display = _display*_mun;
			for(var i = _display-1; i < _display+6; i++){
				$this.parent().find('li').eq(i).addClass('hover');
				_iSlengthHover();
			}
		});
	}

	dealerLod('.J_moreRecommend1', _mun1);
	dealerLod('.J_moreRecommend2', _mun2);
	dealerLod('.J_moreRecommend3', _mun3);*/
